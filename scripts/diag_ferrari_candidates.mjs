#!/usr/bin/env node
// DIAGNOSTIC — dump raw image candidates from a few representative Ferrari
// model pages so we can see whether static fetch surfaces anything usable
// (and what URL/alt patterns model imagery uses if so). Ferrari is JS-rendered
// per Session 5/6 evidence (10 of 11 Playwright escalations returned 0
// candidates); this static dump tells us if the static path is also empty.
//
// Companion: scripts/diag_ferrari_playwright.mjs does the same dump from the
// rendered DOM.
//
// Logs to: reports/ferrari_candidates_raw.log
//
...
const OUT_PATH = path.join(PROJECT_ROOT, "reports", "ferrari_candidates_raw.log");
...
// Representative pages to probe — these are the canonical URLs from the
// Phase B-updated ferrari.json. Mix bare-slug pages (296-gtb, f80) and
// prefix-slug pages (ferrari-roma-spider, ferrari-amalfi, ferrari-12cilindri,
// ferrari-purosangue) so any URL-pattern differences surface.
const TARGETS = [
...
// Read brand config so we replay the actual slug matching the scraper does.
...
async function main() {
  ...
  log(`# Ferrari candidate dump (STATIC FETCH) — generated ${new Date().toISOString()}`);
  ...
  for (const t of TARGETS) {
    ...
    // Cross-model name mentions on this page (would suggest a hub/index page)
    const allModelNames = ["roma","amalfi","296","12cilindri","testarossa","purosangue","f80","sf90"];
