...
// Jeep model pages, plus whether the current slug_variants config would match
// each candidate. Modeled on scripts/diag_mercedes_candidates.mjs.
//
// Usage: node scripts/diag_jeep_candidates.mjs
...
async function main() {
  const CONFIG = path.join(PROJECT_ROOT, "scripts", "brand-configs", "jeep.json");
  ...
  // Hint targets from brief: Wrangler, Grand Cherokee, Compass, Wagoneer.
  // Add gladiator + grand-cherokee-l for trim-name signal coverage.
  const TARGETS = ["wrangler", "grand-cherokee", "compass", "grand-wagoneer", "gladiator", "grand-cherokee-l"];
