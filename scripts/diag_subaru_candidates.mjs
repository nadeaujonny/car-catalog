#!/usr/bin/env node
// DIAGNOSTIC — dump raw image candidates from a few representative Subaru
// model pages.
//
// Phase-C result: 10 of 10 pages escalated to Playwright (static fetch returned
// 0 raw candidates), Playwright surfaced 416–636 candidates per page, but only
// 9 of 131 image entries (6.9%) ended up resolved — and all 9 came via the
// positional-fallback path (the `[pos]` tag in the scrape summary), only for
// the front_three_quarter angle on BRZ and WRX. Zero angle-pattern matches.
//
// Logs to: reports/subaru_candidates_raw.log
//
...
// sees in production. Mirrors scripts/diag_hyundai_candidates.mjs structure.
//
// Note: Subaru pages return 0 candidates on a static fetch (the per-model HTML
// is generated entirely client-side). For meaningful URL data we MUST go via
// Playwright. The companion script scripts/diag_subaru_playwright.mjs does the
// JS-rendered probe and is the primary source of truth for pattern derivation.
// This script keeps the static probe in case the page contents change.
...
const OUT_PATH = path.join(PROJECT_ROOT, "reports", "subaru_candidates_raw.log");
...
// Representative pages to probe (per task hint)
const TARGETS = [
  { slug: "outback",   url: "https://www.subaru.com/vehicles/outback"   },
  { slug: "forester",  url: "https://www.subaru.com/vehicles/forester"  },
  { slug: "crosstrek", url: "https://www.subaru.com/vehicles/crosstrek" },
  { slug: "solterra",  url: "https://www.subaru.com/vehicles/solterra"  },
  { slug: "wrx",       url: "https://www.subaru.com/vehicles/wrx"       },
];
...
async function readBrandSlugVariants() {
  const cfgPath = path.join(PROJECT_ROOT, "scripts", "brand-configs", "subaru.json");
...
async function main() {
  ...
  log(`# Subaru candidate dump (STATIC fetch) — generated ${new Date().toISOString()}`);
  log(`# Loaded slug_variants for: ${Object.keys(slugVariants).join(", ") || "(none — subaru.json declares slug_variants: {})"}`);
  log(`# NOTE: subaru.com per-model pages return JS-rendered HTML; static fetch is expected to return 0 candidates.`);
  log(`#       See scripts/diag_subaru_playwright.mjs for the JS-rendered probe (primary source of truth).`);
  log("");
  ...
  for (const t of TARGETS) {
    ...
    const allModelNames = ["impreza","crosstrek","forester","outback","ascent","brz","wrx","solterra","trailseeker","uncharted"];
    ...
    log(`# any-model-name mentions on this page (URL+alt):`);
