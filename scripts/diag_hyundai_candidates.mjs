#!/usr/bin/env node
// DIAGNOSTIC — dump raw image candidates from a few representative Hyundai
// model pages so we can see why slugMatchesURL is rejecting everything.
//
// Phase-C result: 14 pages produced 95-457 raw candidates each but ZERO of
// them matched any of the page's model slug variants. Need to look at the
// actual URL paths + alt text the pages emit to know whether the fix is
// slug-variants (config) or something deeper (no per-model imagery).
//
// Logs to: reports/hyundai_candidates_raw.log
//
...
const OUT_PATH = path.join(PROJECT_ROOT, "reports", "hyundai_candidates_raw.log");
...
// Read brand config so we can replay the actual slug matching the scraper does.
async function readBrandSlugVariants() {
  const cfgPath = path.join(PROJECT_ROOT, "scripts", "brand-configs", "hyundai.json");
...
async function main() {
  ...
  log(`# Hyundai candidate dump — generated ${new Date().toISOString()}`);
  ...
  for (const t of TARGETS) {
    ...
    // Alt-text snippets that contain the model slug bareword (or chassis code)
    const slug = t.slug.toLowerCase();
    const chassisMap = { tucson: "nx4", elantra: "cn7", "ioniq-5": "ne", "santa-fe": "tm" };
    const chassis = chassisMap[slug];
    const altsWithSlug = cands.filter(c => {
      const hay = (c.context || "").toLowerCase();
      if (hay.includes(slug)) return true;
      if (hay.includes(slug.replace(/-/g, ""))) return true;
      if (chassis && (c.url + " " + (c.context||"")).toLowerCase().includes(chassis)) return true;
      return false;
    });
    log(`# candidates whose URL or alt contains the model name / chassis code: ${altsWithSlug.length}`);
    ...
    // Also explicitly enumerate any candidate alt that mentions ANY model name
    // — would suggest a hub page with mixed model imagery.
    const allModelNames = ["venue","elantra","sonata","kona","tucson","santa","palisade","ioniq","nexo"];
